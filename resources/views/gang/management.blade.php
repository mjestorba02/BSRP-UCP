    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <li>
            <div class="flex items-center">
                ...
                <a href="#" class="ml-1 text-sm font-medium text-white hover:text-gray-300 md:ml-2">Gang Members</a>
            </div>
        </li>
    </ol>
</div>
...
<div class="p-6">
    <h1 class="text-xl text-white font-bold mb-4">Gang Members</h1>
    <div class="max-x-full mx-auto p-6 bg-gradient-to-r from-black to-[#410000] text-white rounded-xl shadow-lg space-y-8">

        @php
            $members_count = app('App\Http\Controllers\GangController')->countGangMembers($gang->id);
        @endphp

        <!-- Gang Info -->
        <div class="bg-red-900/20 p-4 rounded-lg space-y-2">
            <h1 class="text-xl text-white font-bold"></strong> {{ $gang->name }}</h1>
            <p class="text-gray-400"><strong>Members:</strong> <span class="text-white">{{ $members_count }}</span></p>
            <p class="text-gray-400"><strong>MOTD:</strong> <span class="text-white">{{ $gang->motd }}</span></p>
        </div>
                ...
                <table class="min-w-full text-sm text-left">
                    ...
                    <tbody class="divide-y divide-red-900">
                        @forelse($members as $member)
                            @php
                                $gangRank = $gangranks
                                    ->first(fn($r) => $r->id == $member->gang && $r->rank == $member->gangrank)?->name ?? 'None';
                            @endphp
                            <tr class="hover:bg-red-900 transition duration-150">
                                <td class="px-4 py-3 flex items-center space-x-3">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <span class="font-medium text-white">{{ str_replace('_', ' ', $member->username) }}</span>
                                        @if ($member->glockerl)
                                ...
                                <td class="px-4 py-3 text-center">
                                    @if ($user->gangrank == 6 && $member->uid !== $user->uid)
                                        <form action="{{ route('gang.updateRank', $member->uid) }}" method="POST">
                                            @csrf
                                            <select name="gangrank" onchange="this.form.submit()" class="bg-red-800 text-white text-sm rounded p-1">
                                                @foreach($gangranks->where('id', $member->gang) as $rank)
                                                    <option value="{{ $rank->rank }}" @selected($rank->rank == $member->gangrank)>
                                                        {{ $rank->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <form action="{{ route('gang.kick', $member->uid) }}" method="POST"
                                        onsubmit="return confirm('Are you sure you want to kick this member?');">
                                        @csrf
                                        @method('DELETE')
                                        <button class="bg-red-600 hover:bg-red-700 text-white text-xs px-3 py-1.5 rounded transition">
                                            Kick
                                        </button>
                                    </form>
                                </td>
